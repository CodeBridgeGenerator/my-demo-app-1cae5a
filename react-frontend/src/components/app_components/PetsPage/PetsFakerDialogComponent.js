...
import petsFakerFactory from "./petsFakerFactory";
...
const PetsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = petsFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("pets").create(elem));
...
export default connect(mapState, mapDispatch)(PetsFakerDialogComponent);
