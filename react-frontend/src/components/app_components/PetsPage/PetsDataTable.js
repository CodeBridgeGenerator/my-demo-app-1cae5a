...
const PetsDataTable = ({ items, fields, onEditRow, onRowDelete, onRowClick, searchDialog, setSearchDialog,   showUpload, setShowUpload,
...
const pTemplate1 = (rowData, { rowIndex }) => <p >{rowData.animaltype}</p>
...
<Column field="animaltype" header="Animaltype" body={pTemplate1} filter={selectedFilterFields.includes("animaltype")} hidden={selectedHideFields?.includes("animaltype")}  sortable style={{ minWidth: "8rem" }} />
      ...
      {selectedItems.length > 0 ? (
        ...
        <Dialog header="Upload Pets Data" visible={showUpload} onHide={() => setShowUpload(false)}>
        <UploadService 
          user={user} 
          serviceName="pets"            
          onUploadComplete={() => {
      ...
      <Dialog header="Search Pets" visible={searchDialog} onHide={() => setSearchDialog(false)}>
      Search
    </Dialog>
    <Dialog
        header="Filter Users"
